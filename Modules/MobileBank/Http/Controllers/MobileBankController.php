...
class MobileBankController extends BaseController
{
    ...
    public function index()
    {
        if(permission('mobile-bank-access')){
            $this->setPageData('Manage Mobile Bank','Manage Mobile Bank','fas fa-mobile-alt',[['name' => 'Manage Mobile Bank']]);
        ...
        if($request->ajax()){
            if(permission('mobile-bank-access')){
                ...
                foreach ($list as $value) {
                    ...
                    $row[] = number_format($this->bank_balance($value->id),2);
            ...
            if(permission('mobile-bank-add') || permission('mobile-bank-edit')){
                ...
                try {
                    ...
                    if(empty($request->update_id)){
                        ...
                        $bank_coa['level']             = 4;
                        $bank_coa['type']              = 'A';
                        $bank_coa['is_transaction']    = 1;               //1=yes,2=no
    ...
    private function bank_balance(string $mobile_bank_id){
        $data = DB::table('transactions as t')
                ...
                ->groupBy('coa.name')
                ->where('coa.mobile_bank_id',$mobile_bank_id)
                ->where('t.approve',1)
                ->first();
