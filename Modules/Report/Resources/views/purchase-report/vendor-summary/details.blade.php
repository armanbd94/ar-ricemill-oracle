        ...
        <div class="card card-custom custom-card">
            <div class="card-header flex-wrap p-0">
                <div class="card-toolbar m-0">
                    ...
                    <a href="{{ url('purchase-by-vendor-summary') }}" class="btn btn-warning btn-sm font-weight-bolder custom-btn">
                        ...
                        <x-form.selectbox labelName="Vendor" name="vendor_id" col="col-md-4" class="selectpicker">
                            @if (!$vendors->isEmpty())
                                @foreach ($vendors as $value)
                                    <option value="{{ $value->id }}" {{ $vendor->id == $value->id ? 'selected' : '' }}>{{ $value->trade_name.' - '.$value->name.' - '.$value->address }}</option>
                                @endforeach
                            @endif
                        </x-form.selectbox>
...
function report_data()
{
    ...
    if (start_date && end_date && vendor_id) {

        $.ajax({
            url:"{{ route('purchase.vendor.summary.details.data') }}",
            type:"POST",
