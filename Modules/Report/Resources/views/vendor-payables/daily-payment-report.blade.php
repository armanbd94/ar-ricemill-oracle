                    ...
                    <div class="row">
                        <div class="form-group col-md-4">
                            ...
                            <div class="input-group">
                                <input type="text" class="form-control daterangepicker-filed" value="{{ date('d-m-Y').' to '.date('d-m-Y') }}">
                                <input type="hidden" id="start_date" name="start_date" value="{{ date('d-m-Y') }}">
                        ...
                        <x-form.selectbox labelName="Vendor" name="vendor_id" col="col-md-4" class="selectpicker">
                            @if (!$vendors->isEmpty())
                                @foreach ($vendors as $value)
                                    <option value="{{ $value->id }}">{{ $value->trade_name.' - '.$value->name }}</option>
                                @endforeach
                            @endif
                        </x-form.selectbox>
...
function report_data()
{
    ...
    if (start_date && end_date) {
        ...
        $.ajax({
            url:"{{ route('daily.payment.report.data') }}",
            type:"POST",
