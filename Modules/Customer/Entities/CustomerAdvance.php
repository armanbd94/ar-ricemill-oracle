...
class CustomerAdvance extends BaseModel
{
    ...
    protected $fillable = ['chart_of_account_id', 'voucher_no', 'voucher_type', 'voucher_date',
    ...
    *******************************************/
    ...
    private function get_datatable_query()
    {
        ...
        $this->column_order = ['transactions.id','c.name', 'c.trade_name','c.mobile',null,null,'transactions.created_at',null,null,null];
        
        
        $query = self::select('transactions.*','coa.id as coa_id','coa.code','c.id as customer_id','c.name as customer_name','c.trade_name','c.mobile')
    ...
    {
        return self::select('transactions.*','coa.id as coa_id','coa.code','c.id as customer_id','c.name','c.trade_name','c.mobile')
        ->join('chart_of_accounts as coa','transactions.chart_of_account_id','=','coa.id')
        ->join('customers as c','coa.customer_id','c.id')
        ->where([
            'transactions.voucher_type' => self::TYPE,
