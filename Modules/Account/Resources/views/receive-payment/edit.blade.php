                    ...
                    <form id="receive-payment-form" method="post">
                        ...
                        <div class="row">
                            <div class="col-md-12">
                                ...
                                <x-form.selectbox labelName="Customer" name="customer_id"  onchange="dueAmount(this.value)" required="required"  col="col-md-6" class="selectpicker">
                                    @if (!$customers->isEmpty())
                                        @foreach ($customers as $customer)
                                            <option value="{{ $customer->id }}" {{ $voucher_data[0]->chart_of_account_id == $customer->coa->id ? 'selected' : '' }}>{{ $customer->trade_name.' - '.$customer->name }}</option>
                                        @endforeach
                                    @endif
                                </x-form.selectbox>
                                ...
                                <x-form.textbox labelName="Amount" name="amount" required="required" col="col-md-6" value="{{ $voucher_data[0]->credit }}" />
...
function store_data(){
    ...
    let url = "{{url('receive-payment/update')}}";
    $.ajax({
        url: url,
        type: "POST",
        data: formData,
        ...
        success: function (data) {
            ...
            if (data.status == false) {
                ...
                if (data.status == 'success' && data.vendor_transaction != '') {
                    ...
                    window.location.replace("{{ url('receive-payment') }}");
